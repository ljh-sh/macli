...
package_target() {
    ...
    local tarball="$OUT_DIR/macli-darwin-${arch_name}.tar.xz"
    ...
    # Deterministic tarball (bsdtar on macOS). Internal layout: bin/macli
    (cd "$pkg_dir" && \
        tar -cJf "$OUT_DIR/macli-darwin-${arch_name}.tar.xz" \
            --format ustar \
            bin)
...
verify_reproducibility() {
    ...
    shasum -a 256 "$OUT_DIR/$target/bin/macli" > /tmp/macli-verify-h1.txt

    do_build "$arch" "darwin" "${target#darwin-}"
    shasum -a 256 "$OUT_DIR/$target/bin/macli" > /tmp/macli-verify-h2.txt

    if diff /tmp/macli-verify-h1.txt /tmp/macli-verify-h2.txt >/dev/null; then
        echo "✓ PASS: same sha256 across two consecutive builds"
        cat /tmp/macli-verify-h1.txt
    else
        echo "✗ FAIL: sha256 differs"
        diff /tmp/macli-verify-h1.txt /tmp/macli-verify-h2.txt
        exit 1
    fi

    # RPATH leak check
    if otool -l "$OUT_DIR/$target/bin/macli" | grep -A2 "LC_RPATH" | grep -q "/Library/Developer"; then
        echo "✗ FAIL: build-machine RPATH leaked"
        otool -l "$OUT_DIR/$target/bin/macli" | grep -A2 "LC_RPATH" | grep "/Library/Developer"
        exit 1
    fi
    echo "✓ PASS: no build-machine RPATH"

    # User path leak check
    if strings "$OUT_DIR/$target/bin/macli" | grep -qE "^/Users/|^/home/"; then
