...
#### Optimized Battery Charging 的学习机制与决策

**它是怎么学习的？**

1. macOS 会记录你每天的插电和拔电时间、使用时长。
2. 当它发现你有规律（比如每晚 23:00 插电，早上 8:30 拔掉），它会：
   - 先快速充到 **80%**；
   - 在 80% 处“暂停”；
   - 在你预计拔掉电源前不久（比如早上 7:30）再把最后 20% 充满。
3. 这样既减少了电池在高电压下停留的时间，又能保证你拔电时电池是满的。

**它的局限：**

- **作息不规律时基本无效**。如果你每天插电/拔电时间都不一样，系统学不出模式，就会直接充满到 100%。
- **学习需要几天甚至几周**。新机器或刚升级系统后，可能要一段时间才开始生效。
- **最终还是会到 100%**。它不是硬上限，只是“延迟充满”。

**决策建议：**

| 你的情况 | 推荐做法 |
|---|---|
| 每天固定时间插电、固定时间拔电 | 可以交给 **Optimized Battery Charging** 自动管理 |
| 作息不规律、经常出差、随时可能拔电 | 建议开 **80% Limit / Charging Limit** 硬上限 |
| 长期接显示器当台式机，几乎不带走 | 建议用 **80% Limit** 或第三方工具设到 **60%–70%** |

> 临时需要充满时，可以点击电池菜单或系统通知里的 **“Charge to Full Now”**（立即充满），让系统跳过学习策略一次。
...
**80% 是大多数人的甜点，但不是所有人的最优解。**

锂电池在高 SOC（高电压）下老化更快。80% 已经比 100% 好很多；如果你**几乎 24 小时插电**（比如 Mac 长期接显示器当台式机用），把上限进一步降到 **60%–70%** 可以减少化学应力、延缓容量衰减。代价是断电时的可用续航变少。

| 使用方式 | 推荐上限 | 理由 |
|---|---|---|
| 经常外出 / 移动办公 | 100% 或 80% | 需要最大可用续航 |
| 偶尔移动， mostly 插电 | 80% | 苹果官方甜点，续航与寿命平衡 |
| 长期当台式机用，几乎不带走 | 60%–70% | 进一步延长电池寿命，但牺牲应急续航 |

macOS 原生只支持 **80%–100%** 的可调范围，所以想要 **60%–70%** 必须用 `batt`（Apple Silicon）或 `AlDente` 等第三方工具。
...
`macli battery` 给了一堆数字，但数字背后的决策更重要。下面是一些实用的电池知识，帮你判断该怎么用、该怎么设。

### 循环次数 vs 健康度

- **循环次数**（`cycleCount`）只是老化指标之一，不是全部。
- **健康度**（`healthPercent`）反映的是当前最大容量相对于设计容量的比例，是化学老化的综合结果。
- 一个循环次数很低但长期在 100% 满电、高温环境下使用的电池，健康度可能掉得很快；反过来，循环次数高但使用温和的电池，健康度可能保持得更好。

```sh
# 同时看循环次数和健康度
macli battery | jq '{cycleCount, designCycleCount, healthPercent, temperature}'
```

### 温度比循环次数更伤电池

锂电池最怕的是**高温**，其次是**长期高电量**。

- 理想工作温度：约 **10°C–35°C**。
- 长期高于 35°C 会加速电解液分解和容量衰减。
- 常见的高温场景：被子/沙发上堵住散热口、夏天阳光直射、边充电边跑高强度编译/游戏。

```sh
# 看看你的电池现在热不热
macli battery | jq '.temperature'
```

### 充电习惯：浅充浅放更好

- **不需要等到 0% 再充**。锂电池没有记忆效应，浅充浅放反而更友好。
- 长期在 **20%–80%** 之间循环，化学应力最小。
- 长期插电建议开启 **80% Limit**；如果几乎不移动，可考虑 **60%–70%**。
...
现代 Mac 的电量计（gas gauge）会**自动校准**，大多数时候不需要你手动做完整充放电循环。

只在出现以下现象时考虑校准：

- 电量百分比突然跳变（比如从 30% 直接跳到 5%）；
- 显示 100% 后很快掉电；
- `healthPercent` 在不同次采样间大幅波动。

校准方法：用到接近自动关机，然后一次性充满到 100%，中间不要中断。

### 什么时候该考虑换电池

| 信号 | 说明 |
|---|---|
| `healthPercent` < 80% | 容量已明显衰减 |
| `cycleCount` 接近或超过 `designCycleCount` | 常见设计寿命是 1000 次 |
| 机身鼓包 | 立即停止使用并更换 |
| 意外关机 | 可能是电池内阻过大或保护板动作 |
| 系统显示 **Service Recommended** | 官方建议维修 |
| `cellVoltageDelta` 持续 > 0.05 V | 电芯不平衡加剧 |
...
如果你开了 80% Limit，MagSafe 灯在 80% 时就会变绿，这是正常的。

### 常见误区

- ❌ **“必须把电用完再充”** — 锂电池没有记忆效应，深度放电反而有害。
- ❌ **“快充伤电池”** — 充电速度由 BMS 控制，真正伤电池的是快充时产生的高温。
- ❌ **“一直插电会充坏电池”** — 现代 BMS 会停充并转由市电供电，但长期停在 100% 会加速化学老化；开 80% Limit 可缓解。
- ❌ **“循环次数低就一定健康”** — 高温、满电存放、电芯不平衡同样会拉低健康度。
